...
doWrite(z_params& zs, Flush flush, error_code& ec)
{
    ...
    auto const done =
        [&]
        {
            ...
            if(wrap(wrap_ % 128) !=boost::deflate::wrap::none) {
                check_ = (wrap(wrap_ % 128) == boost::deflate::wrap::zlib) ?
    ...
    for(;;)
    {
        ...
        case HEAD: {
            ...
            if (wrap == boost::deflate::wrap::gzip && hold == 0x8b1fU) {
            ...
            BOOST_ASSERT(wrap == boost::deflate::wrap::zlib);
        ...
        case CHECK:
            ...
            BOOST_FALLTHROUGH;
        case LENGTH:
            if(wrap_ / 128 && wrap(wrap_ % 128) == boost::deflate::wrap::gzip) {
                if(!bi_.fill(32, r.in.next, r.in.last))
                    return done();
                std::uint32_t hold;
                bi_.read_all(hold);
...
{
    if((windowBits == 0 && wrap == boost::deflate::wrap::zlib)
